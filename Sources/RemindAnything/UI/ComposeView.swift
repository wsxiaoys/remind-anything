        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            .onChange(of: relativePreset) { _, newValue in
                ...
                case .custom:
                    ...
                    draft.scheduleKind = .absolute
                    draft.absoluteDate = Self.roundedUpToQuarterHour(draft.absoluteDate)
                }
            }
            ...
            if relativePreset == .custom {
                HStack(spacing: 8) {
                    PickerField(icon: "calendar",
                                text: draft.absoluteDate.formatted(date: .abbreviated, time: .omitted)) { _ in
                        DatePicker("", selection: $draft.absoluteDate, in: Date()...,
                    ...
                    PickerField(icon: "clock",
                                text: draft.absoluteDate.formatted(date: .omitted, time: .shortened)) { dismiss in
                        TimeListPicker(selection: $draft.absoluteDate, onPick: dismiss)
                    }
                    Spacer(minLength: 0)
                }
            } else {
