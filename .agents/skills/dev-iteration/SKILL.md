...
- **CRITICAL: Recording feedback is a strictly isolated phase from implementing or researching. While collecting/recording feedback, you MUST NOT read code, search files, investigate the codebase, or make any code changes. Your sole task is to transcribe the user's report clearly and fluently into `feedback.md` describing only the user's issue, its visual or functional impact, and any attached screenshots.
  - **Handling Screenshots & Assets**: When the user provides screenshots (typically as remote Pochi upload links or other URLs), **you MUST NOT reference the remote URLs directly** in `feedback.md`. Instead, you must fetch/download each image (e.g., using `curl -L` or another appropriate command) and save it locally inside the `docs/iteration-<N>/assets/` directory (creating it if it doesn't exist). Then, reference the local asset using a relative path (e.g., `assets/some-description.png`) in `feedback.md`.
...
- **Screenshots and Assets**: Always download screenshots provided by the user and save them in the local `docs/iteration-<N>/assets/` folder. Reference them in `feedback.md` using relative paths. This ensures the document remains self-contained and does not rely on transient remote links.
